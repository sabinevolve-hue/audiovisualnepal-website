...
function fullPageSlugs(): Set<string> {
  return new Set(
    ALL_PRODUCTS.filter((p) => p.brandSlug === "tenveo").map((p) =>
      p.name.replace(/^Tenveo\s+/i, "").toLowerCase()
    )
  );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const desc = e.desc || `Tenveo ${hit.model} — ${hit.type} in the ${hit.series} series. Available in Nepal from the authorised Tenveo distributor with manufacturer warranty.`;
  return {
    title: `Tenveo ${hit.model} in Nepal — ${hit.type}`,
    description: desc.slice(0, 160),
    alternates: { canonical: `https://audiovisualnepal.com/brands/tenveo/p/${model}` },
    openGraph: { title: `Tenveo ${hit.model} | AudioVisual Nepal`, description: desc.slice(0, 160), images: [{ url: e.img }] },
  };
}
    ...
    <main className="pt-20 bg-white">
      ...
      <section className="mx-auto max-w-6xl px-6 py-12">
        <p className="text-sm font-semibold uppercase tracking-widest text-blue-600">
          <Link href="/brands/tenveo" className="hover:underline">Tenveo</Link>
          {" / "}
          <Link href="/brands/tenveo/catalog" className="hover:underline">Catalog</Link>
          {" / "}{hit.category}
        </p>
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-2">
          <div className="relative flex min-h-[320px] items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 p-8">
            <Image src={e.img} alt={`Tenveo ${hit.model}`} width={640} height={480} className="h-auto max-h-[420px] w-auto object-contain" priority />
          </div>
          <div>
            ...
            <p className="mt-4 text-sm text-slate-500">
              Genuine Tenveo product with full manufacturer warranty, supplied and supported in Nepal
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <a href={`https://wa.me/+9779762109538?text=${encodeURIComponent(`Hello! I'd like a quote for the Tenveo ${hit.model}.`)}`} className="rounded-full border border-slate-300 px-6 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-blue-400">
                WhatsApp us
              </a>
            </div>
            <p className="mt-4 text-xs text-slate-400">Product summary courtesy of Tenveo. Images © Tenveo.</p>
          </div>
        </div>
