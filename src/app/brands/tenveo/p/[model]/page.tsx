...
const CATEGORY_SOLUTIONS = (relations as { categorySolutions: Record<string, Record<string, Rel[]>> }).categorySolutions["tenveo"] || {};
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: desc,
    alternates: { canonical: `https://www.audiovisualnepal.com/brands/tenveo/p/${model}` },
    ...
    <main className="pt-20 bg-white">
      ...
      <section className="mx-auto max-w-6xl px-6 py-12">
        <p className="text-sm font-semibold uppercase tracking-widest text-blue-600">
          <Link href="/brands/tenveo" className="hover:underline">{BRAND}</Link>
          {" / "}
        ...
        {related.length > 0 && (
          <div className="mt-14">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {related.map((m) => {
                const me = MEDIA[m];
                const href = full.has(m.toLowerCase()) ? null : `/brands/tenveo/p/${slugify(m)}`;
                const card = (
        ...
        <div className="mt-14 flex flex-wrap items-center gap-3 rounded-2xl bg-slate-50 p-5 text-sm">
          ...
          <Link href="/brands/tenveo/catalog" className="text-blue-600 hover:underline">Full {BRAND} catalog</Link>
