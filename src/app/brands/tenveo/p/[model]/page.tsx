...
function fullPageSlugs(): Set<string> {
  return new Set(ALL_PRODUCTS.filter((pr) => pr.brandSlug === "tenveo").map((pr) => pr.name.replace(new RegExp(`^${BRAND}\\s+`, "i"), "").toLowerCase()));
}
...
export default async function CatalogProductPage({ params }: Props) {
  ...
  const related = hit.siblings.filter((m) => m !== hit.model).slice(0, 4).map((m) => ({
    name: m,
    img: MEDIA[m]?.img,
    href: full.has(m.toLowerCase()) ? null : `/brands/tenveo/p/${slugify(m)}`,
  }));
  ...
  return (
    <>
      ...
      <CatalogProductShell
        ...
        related={related}
        brandHref="/brands/tenveo"
        catalogHref="/brands/tenveo/catalog"
      />
    </>
  );
}
