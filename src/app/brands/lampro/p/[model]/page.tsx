    ...
    <main className="pt-20 bg-white">
      ...
      <section className="mx-auto max-w-6xl px-6 py-12">
        <p className="text-sm font-semibold uppercase tracking-widest text-blue-600">
          <Link href="/brands/lampro" className="hover:underline">{BRAND}</Link>
          {" / "}
          <Link href="/brands/lampro/catalog" className="hover:underline">Catalog</Link>
          {" / "}{hit.category}
        </p>
        ...
        {related.length > 0 && (
          <div className="mt-14">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {related.map((m) => {
                const me = MEDIA[m];
                const href = full.has(m.toLowerCase()) ? null : `/brands/lampro/p/${slugify(m)}`;
                const card = (
        ...
        <div className="mt-14 flex flex-wrap items-center gap-3 rounded-2xl bg-slate-50 p-5 text-sm">
          ...
          <Link href="/brands/lampro/catalog" className="text-blue-600 hover:underline">Full {BRAND} catalog</Link>
