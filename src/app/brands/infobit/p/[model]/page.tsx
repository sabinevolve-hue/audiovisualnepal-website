...
function fullPageSlugs(): Set<string> {
  return new Set(ALL_PRODUCTS.filter((pr) => pr.brandSlug === "infobit").map((pr) => pr.name.replace(new RegExp(`^${BRAND}\\s+`, "i"), "").toLowerCase()));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: desc,
    alternates: { canonical: `https://www.audiovisualnepal.com/brands/infobit/p/${model}` },
...
export default async function CatalogProductPage({ params }: Props) {
  ...
  const related = hit.siblings.filter((m) => m !== hit.model).slice(0, 4).map((m) => ({
    name: m,
    img: MEDIA[m]?.img,
    href: full.has(m.toLowerCase()) ? null : `/brands/infobit/p/${slugify(m)}`,
  }));
  ...
  return (
    <>
      ...
      <CatalogProductShell
        ...
        related={related}
        brandHref="/brands/infobit"
        catalogHref="/brands/infobit/catalog"
      />
    </>
  );
}
