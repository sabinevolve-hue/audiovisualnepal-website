...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const desc = e.desc || `InfoBit ${hit.model} — ${hit.type} in the ${hit.series} series. Available in Nepal from the authorised InfoBit distributor with manufacturer warranty.`;
  return {
    title: `InfoBit ${hit.model} in Nepal — ${hit.type}`,
    description: desc.slice(0, 160),
    alternates: { canonical: `https://audiovisualnepal.com/brands/infobit/p/${model}` },
    openGraph: { title: `InfoBit ${hit.model} | AudioVisual Nepal`, description: desc.slice(0, 160), images: [{ url: e.img }] },
  };
}
    ...
    <main className="pt-20 bg-white">
      ...
      <section className="mx-auto max-w-6xl px-6 py-12">
        <p className="text-sm font-semibold uppercase tracking-widest text-blue-600">
          <Link href="/brands/infobit" className="hover:underline">InfoBit</Link>
          {" / "}
          <Link href="/brands/infobit/catalog" className="hover:underline">Catalog</Link>
          {" / "}{hit.category}
        </p>
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-2">
          <div className="relative flex min-h-[320px] items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 p-8">
            <Image src={e.img} alt={`InfoBit ${hit.model}`} width={640} height={480} className="h-auto max-h-[420px] w-auto object-contain" priority />
          </div>
          <div>
            ...
            <p className="mt-4 text-sm text-slate-500">
              Genuine InfoBit product with full manufacturer warranty, supplied and supported in Nepal
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <a href={`https://wa.me/+9779762109538?text=${encodeURIComponent(`Hello! I'd like a quote for the InfoBit ${hit.model}.`)}`} className="rounded-full border border-slate-300 px-6 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-blue-400">
                WhatsApp us
              </a>
            </div>
            <p className="mt-4 text-xs text-slate-400">Product summary courtesy of InfoBit. Images © InfoBit.</p>
          </div>
        </div>
