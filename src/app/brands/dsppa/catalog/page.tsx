...
const PAGE_URL = "https://audiovisualnepal.com/brands/dsppa/catalog";
                        ...
                        {g.models.some((m) => MEDIA[m]) && (
                          <div className="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-3">
                            {g.models.filter((m) => MEDIA[m]).map((m) => {
                              ...
                              const href = full || `/brands/dsppa/p/${MEDIA[m].slug}`;
                        ...
                        {g.models.some((m) => !MEDIA[m]) && (
                          <p className="mt-2 text-sm leading-relaxed text-slate-700">
                            {g.models.filter((m) => !MEDIA[m]).map((m, i, arr) => {
                              ...
                              const hot = (g as GroupX).featuredModels?.includes(m) || (g as GroupX).featured;
                              const lite = `/brands/dsppa/p/${slug(m)}`;
                              const inner = href ? (
      ...
      <section className="mx-auto max-w-6xl px-6 pb-16 text-center">
        <div className="rounded-2xl bg-slate-50 p-8">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm text-slate-600">
            Send us the model number — we quote any DSPPA product with genuine manufacturer warranty.
          </p>
