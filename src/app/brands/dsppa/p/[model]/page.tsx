...
const BRAND = "DSPPA";
const BRAND_COLOR = "#00AEAD";

type Props = { params: Promise<{ model: string }> };
const slugify = (s: string) => s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");

function fullPageSlugs(): Set<string> {
  return new Set(ALL_PRODUCTS.filter((pr) => pr.brandSlug === "dsppa").map((pr) => pr.name.replace(new RegExp(`^${BRAND}\\s+`, "i"), "").toLowerCase()));
}
...
export default async function CatalogProductPage({ params }: Props) {
  ...
  const related = hit.siblings.filter((m) => m !== hit.model).slice(0, 4).map((m) => ({
    name: m,
    img: MEDIA[m]?.img,
    href: full.has(m.toLowerCase()) ? null : `/brands/dsppa/p/${slugify(m)}`,
  }));
  ...
  return (
    <>
      ...
      <CatalogProductShell
        ...
        related={related}
        brandHref="/brands/dsppa"
        catalogHref="/brands/dsppa/catalog"
      />
    </>
  );
}
